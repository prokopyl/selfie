...
/// # Example
///
/// This example stores both an owned `String` and a mutable [`str`] slice pointing
/// into it, and then only mutates the substring.
///
/// ```
/// use core::pin::Pin;
    ...
    R: for<'this> RefType<'this>,
{
    ...
    ///
    /// let data = Pin::new("Hello, world!".to_owned());
    /// let selfie: Result<SelfieMut<String, Mut<str>>, SelfieError<Pin<String>, ()>> =
    ///     SelfieMut::try_new(data, |s| Ok(&mut Pin::into_inner(s)[0..5]));
    ///
    /// selfie.unwrap().with_referential(|r| assert_eq!("Hello", *r));
    /// ```
    #[inline]
    pub fn try_new<E, F>(mut owned: Pin<P>, handler: F) -> Result<Self, SelfieError<Pin<P>, E>>
    where
    ...
    /// ```
    #[inline]
    pub fn map<R2: for<'this> RefType<'this>, F>(self, mapper: F) -> SelfieMut<'a, P, R2>
    where
        F: for<'this> FnOnce(
            <R as RefType<'this>>::Ref,
    ...
    pub fn try_map<R2: for<'this> RefType<'this>, E, F>(
        self,
        mapper: F,
    ) -> Result<SelfieMut<'a, P, R2>, SelfieError<Pin<P>, E>>
    where
        F: for<'this> FnOnce(
            <R as RefType<'this>>::Ref,
            &'this (), // This is needed to constrain the lifetime TODO: find a way to remove this
        ) -> Result<<R2 as RefType<'this>>::Ref, E>,
    {
        ...
        Ok(SelfieMut { owned, referential })
    }
}
